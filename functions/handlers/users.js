...
exports.postDadosDesempenho = (req, res) => {
  ...
  aluno['Pesos.' + year + '.' + semestre + '.' + monthNumber] = Number(
    req.body.peso
  )
  aluno['IMCs.' + year + '.' + semestre + '.' + monthNumber] = Number(
    req.body.imc
  )
  aluno['BFs.' + year + '.' + semestre + '.' + monthNumber] = Number(
    req.body.bf
  )
  aluno['PCCQs.' + year + '.' + semestre + '.' + monthNumber] =
    Number(req.body.cintura) /
    (Number(req.body.quadril) === 0 ? 1 : Number(req.body.quadril))
  aluno['ITMs.' + year + '.' + semestre + '.' + monthNumber] = Number(
    (Number(req.body.bracoTenso) /
      (Number(req.body.bracoRelax) === 0 ? 1 : Number(req.body.bracoRelax))) *
      100
  )
  aluno['Musculares.' + year + '.' + semestre + '.' + monthNumber] = Number(
    req.body.muscular
  )
  aluno['Vo2s.' + year + '.' + semestre + '.' + monthNumber] = Number(
    req.body.vo2
  )
  aluno['Flexibilidades.' + year + '.' + semestre + '.' + monthNumber] = Number(
    req.body.flex
  )
  aluno['Agachamentos.' + year + '.' + semestre + '.' + monthNumber] = Number(
    req.body.agachamento
  )
  aluno['Abdomens.' + year + '.' + semestre + '.' + monthNumber] = Number(
    req.body.abdomen
  )
  aluno['Apoios.' + year + '.' + semestre + '.' + monthNumber] = Number(
    req.body.apoio
  )
  ...
  ;(aluno.pccqInicial =
    Number(req.body.pccqInicial) ||
    Number(req.body.cintura) /
      (Number(req.body.quadril) === 0 ? 1 : Number(req.body.quadril))),
    (aluno.itmInicial =
      Number(req.body.itmInicial) ||
      Number(
        (Number(req.body.bracoTenso) /
          (Number(req.body.bracoRelax) === 0
            ? 1
            : Number(req.body.bracoRelax))) *
          100
      )),
    (aluno.muscularInicial =
      Number(req.body.muscularInicial) || Number(req.body.muscular))
  ...
  aluno.agachamentoInicial =
    Number(req.body.agachamentoInicial) || Number(req.body.agachamento)
  aluno.abdomenInicial =
    Number(req.body.abdomenInicial) || Number(req.body.abdomen)
...
exports.blockAluno = (req, res) => {
  ...
  admin
    ...
    .then(userRecord => {
      ...
      return db
        .doc(`/usuarios/${req.body.id}`)
        .update({ desativado: flip })
    })
    .then(() => {
    ...
    .then(() => {
      if (!flip)
        return res.status(200).json({ message: 'Usuario ativado com sucesso' })
  ...
  if (date.getUTCDate() === 1) {
    db.collection('usuarios')
      ...
      .then(() => {
        return res
          .status(200)
          .json({ message: 'Scheduled job executed successfully' })
      })
